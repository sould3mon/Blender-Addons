...
class  Pre_bake(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        def ConnectNewNodes(matname,nodeout,nodein):
            if nodeout == None or nodein == None:
                print("no new material returned")
            else:
                
                tree = bpy.data.materials[matname].node_tree
                tree.links.new(nodeout.outputs['UV'],nodein.inputs['Vector'])
            return
...
class Pre_bake_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        self.layout.operator("node.add_pre_bake_nodes", icon='MESH_CUBE', text="Add Nodes")   
...
class ScriptVariables(bpy.types.PropertyGroup):
    ...
    UVMapTarget :  bpy.props.StringProperty(
        name = "UVMap Target",
        description=" select UVMap BakeTarget Default UVMap.bake",
        default = "UVMap.bake"
    )
